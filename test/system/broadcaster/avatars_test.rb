require "application_system_test_case"

class Broadcaster::AvatarsTest < ApplicationSystemTestCase
  setup do
    @broadcaster_avatar = broadcaster_avatars(:one)
  end

  test "visiting the index" do
    visit broadcaster_avatars_url
    assert_selector "h1", text: "Broadcaster/Avatars"
  end

  test "creating a Avatar" do
    visit broadcaster_avatars_url
    click_on "New Broadcaster/Avatar"

    fill_in "Description", with: @broadcaster_avatar.description
    fill_in "Name", with: @broadcaster_avatar.name
    click_on "Create Avatar"

    assert_text "Avatar was successfully created"
    click_on "Back"
  end

  test "updating a Avatar" do
    visit broadcaster_avatars_url
    click_on "Edit", match: :first

    fill_in "Description", with: @broadcaster_avatar.description
    fill_in "Name", with: @broadcaster_avatar.name
    click_on "Update Avatar"

    assert_text "Avatar was successfully updated"
    click_on "Back"
  end

  test "destroying a Avatar" do
    visit broadcaster_avatars_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Avatar was successfully destroyed"
  end
end
