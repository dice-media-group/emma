require "application_system_test_case"

class Blog::ListsTest < ApplicationSystemTestCase
  setup do
    @blog_list = blog_lists(:one)
  end

  test "visiting the index" do
    visit blog_lists_url
    assert_selector "h1", text: "Blog/Lists"
  end

  test "creating a List" do
    visit blog_lists_url
    click_on "New Blog/List"

    fill_in "Name", with: @blog_list.name
    fill_in "Position", with: @blog_list.position
    click_on "Create List"

    assert_text "List was successfully created"
    click_on "Back"
  end

  test "updating a List" do
    visit blog_lists_url
    click_on "Edit", match: :first

    fill_in "Name", with: @blog_list.name
    fill_in "Position", with: @blog_list.position
    click_on "Update List"

    assert_text "List was successfully updated"
    click_on "Back"
  end

  test "destroying a List" do
    visit blog_lists_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "List was successfully destroyed"
  end
end
